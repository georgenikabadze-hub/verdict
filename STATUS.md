...
> **Read this first.** Big Berlin Hack 2026, **Reonic track**. Read `README.md` for the full submission narrative. Memory at `~/.claude/projects/-Users-georgenikabadze/memory/project_bigberlin_hackathon.md` has the hackathon-meta context (track rules, partner-tech list, side prizes).
...
## (Earlier session notes — kept for archeology, may be stale)
...
1. **`Rheinstraße 12, 12159 Berlin` clipping** — Playwright debug agent completed with the exact root cause + fix recipe. The 25-m sanity check just added likely doesn't catch this case because OSM and Solar are within 25 m. **Apply this fix in `lib/osm/footprint.ts` `scoreCandidate` (around lines 214-225):**
   - Replace `(contains ? 100 : 0)` with `(contains ? (tagBuilding(c.tags) ? 100 : 40) : 0)`. This demotes tagless interior polygons (like the Hinterhaus 109280527 that's stealing the win at Rheinstraße 12).
   - Optionally also fetch nearby `way["highway"]` in the Overpass query and add +20 boost to candidates within 5 m of a street centerline (Vorderhaus is on the street; Hinterhaus is 15 m off it).
   - Root cause detail: Google geocodes "Rheinstraße 12" to a lat/lng INSIDE the Hinterhaus (rear courtyard building, way 109280527, no addr tags). The current `contains=true` +100 bonus is unconditional, so the Hinterhaus beats the actual Vorderhaus (way 109280529, has `addr:housenumber=12/13`).
   - Secondary issue: `sampleHeightMostDetailed` returns undefined for this address, so camera target stays at fallback 38 m → camera ends up looking sideways near ground level. Fix at `CesiumRoofViewInner.tsx:640-661` — re-sample on a `tileLoad` event instead of a single setTimeout.
